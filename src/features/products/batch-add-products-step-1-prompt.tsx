...
import { PRODUCT_BATCH_EXTRACTION_PROMPT } from "./product-batch-import-schema";

export function BatchAddProductsStep1Prompt() {
  const [copied, setCopied] = useState(false);
  const [showHelp, setShowHelp] = useState(false);

  const handleCopy = async () => {
    try {
      await navigator.clipboard.writeText(PRODUCT_BATCH_EXTRACTION_PROMPT);
      setCopied(true);
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-sm font-medium">Get JSON from a supplier catalog</h3>
        <p className="mt-1 text-xs text-muted-foreground">
          Copy the prompt below, open Claude or Gemini in another tab, paste the
          prompt, and attach a supplier catalog or price list (PDF, Excel, or
          image). Copy the JSON from the response and bring it back here.
        </p>
      </div>

      <div className="relative">
        <pre className="max-h-80 overflow-auto rounded-md border bg-muted/40 p-3 text-xs leading-relaxed whitespace-pre-wrap">
          {PRODUCT_BATCH_EXTRACTION_PROMPT}
        </pre>
        <Button
          type="button"
          size="sm"
      ...
      {showHelp ? (
        <ol className="ml-2 list-decimal space-y-1 pl-4 text-xs text-muted-foreground">
          ...
          <li>Paste the prompt, attach the catalog file.</li>
