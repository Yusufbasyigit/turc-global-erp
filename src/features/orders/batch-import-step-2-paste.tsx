...
type ParseState =
  ...
  | { status: "ok"; parsed: ProformaImport };
...
function parseJson(text: string): ParseState {
  ...
  const result = proformaImportSchema.safeParse(raw);
  if (!result.success) {
...
export function BatchImportStep2Paste({
  jsonText,
  onJsonTextChange,
  onValid,
}: {
  ...
  onValid: (parsed: ProformaImport | null) => void;
}) {
    ...
    <div className="space-y-3">
      ...
      >
        ...
        <div>
          ...
          {state.status === "ok" &&
            `Found ${state.parsed.lines.length} line${state.parsed.lines.length === 1 ? "" : "s"}.`}
        </div>
      </div>
    </div>
  );
